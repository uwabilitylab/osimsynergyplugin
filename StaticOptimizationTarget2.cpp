/* -------------------------------------------------------------------------- *
 *                   OpenSim:  StaticOptimizationTarget2.cpp                   *
 * -------------------------------------------------------------------------- *
 ...
 * Author(s): Frank C. Anderson with additions for matrix weights			  *
 *            from Kat M. Steele											  *      
 *                                                                            *
...
#include "StaticOptimizationTarget2.h"
#include <iostream>
...
const double StaticOptimizationTarget2::SMALLDX = 1.0e-14;
//const double StaticOptimizationTarget2::_activationExponent = 2.0;
 ...
 */
StaticOptimizationTarget2::
StaticOptimizationTarget2(const SimTK::State& s, Model *aModel,int aNP,int aNC, bool useMusclePhysiology)
{
...
// CONSTRUCTION
//==============================================================================
bool StaticOptimizationTarget2::
 ...
 * @param aModel Model.
 */
void StaticOptimizationTarget2::
...
setDX(double aValue)
{
	...
	// SET VALUE
	for(int i=0;i<getNumParameters();i++) _dx.set(i, aValue);
}
//______________________________________________________________________________
/**
 * Get the derivative perturbation size.
 */
double StaticOptimizationTarget2::
getDX(int aIndex)
{
...
{
	if(aSize<SMALLDX) {
		printf("StaticOptimizationTarget2.validatePerturbationSize: WARNING- ");
 ...
 */
int StaticOptimizationTarget2::
CentralDifferencesConstraint(const StaticOptimizationTarget2 *aTarget,
 ...
 */
int StaticOptimizationTarget2::
CentralDifferences(const StaticOptimizationTarget2 *aTarget,
...
constraintJacobian(const SimTK::Vector &parameters, const bool new_parameters, SimTK::Matrix &jac) const
{
	...
	// Compute gradient 
	StaticOptimizationTarget2::CentralDifferencesConstraint(this,&_dx[0],parameters,jac);
...
computeAcceleration(SimTK::State& s, const SimTK::Vector &parameters,SimTK::Vector &rAccel) const
{
	...
	// JACKM: Need to talk to sherm if this restriction can be removed							//KAT: Correction?
	...
	for(int i=0,j=0;i<fs.getSize();i++)  {
		 ScalarActuator* act = dynamic_cast<ScalarActuator*>(&fs.get(i));
		 if( act ) {
			 act->setOverrideActuation(s,activ(j,0)*_optimalForce[j]); //KAT: Changed from act->setOverrideForce(s,parameters[j]*_optimalForce[j]);
		 }
         j++;
    }
